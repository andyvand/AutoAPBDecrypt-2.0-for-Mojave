...
-(BOOL)apb_initialize:(BF_KEY *)key aesctx1:(AES_KEY *)ctx1 aesctx2:(AES_KEY *)ctx2
{
    BOOL result = NO;
    ...
    AES_set_decrypt_key(plain_key, 0x100, ctx1);
    AES_set_decrypt_key(plain_key + 0x20, 0x100, ctx2);
...
/*#if MAC_OS_X_VERSION_MIN_REQUIRED < 1080*/
    if (decryptOld == YES)
    {
        AES_cbc_encrypt ((unsigned char *) in, (unsigned char *)out, 0x80, &ctx1 , null_ivec, AES_DECRYPT);
        AES_cbc_encrypt ((unsigned char *) in + 0x800, (unsigned char *)out + 0x800, 0x80, &ctx2, null_ivec, AES_DECRYPT);
    } else {
...
-(void)runDecrypt:(id)sender
{
    ...
    workthread = [[[NSThread alloc] initWithTarget:self selector:@selector(runDecryptThread:) object:nil] autorelease];
